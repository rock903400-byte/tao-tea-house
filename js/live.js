...
(function () {
  ...
  function renderCourse(course) {
    ...
    var pic = img
      ? '<picture><img src="' + esc(img) + '" alt="' + esc(course.title) + ' - 陶茶雅舍" loading="lazy" width="800" height="450" decoding="async"></picture>'
      : '';
    var subtitle = course.subtitle ? '<br><span style="font-size:0.85rem;color:var(--color-soft);font-weight:normal;">（' + esc(course.subtitle) + '）</span>' : '';
    var fee = String(course.fee || '').replace(/^(\$[\d,]+)\s*\/\s*人$/, '<strong style="color:var(--color-accent);font-size:1.15rem;">$1</strong> ／ 人');
    ...
    if (course.location) meta += '<dt>📍 地點</dt><dd>' + esc(course.location.split('（')[0]) + '</dd>';

    var signup = course.signup_url
      ? '<a href="' + esc(course.signup_url) + '" target="_blank" rel="noopener" class="btn btn--primary">⬇️ 立即報名</a>'
      : '';
    return '<article class="course-card fade-in visible">' +
      ...
      '<h3 class="course-card__title">' + esc(course.title) + subtitle + '</h3>' +
      (course.description ? '<p class="course-card__desc">' + esc(course.description) + '</p>' : '') +
      (meta ? '<dl class="course-card__meta">' + meta + '</dl>' : '') +
      '<div style="display:flex;gap:12px;flex-wrap:wrap;">' + signup +
  ...
  function renderTeacher(t) {
    return '<div class="teacher-info fade-in visible">' +
      '<h3 class="teacher-info__name">' + esc(t.name) + '</h3>' +
      (t.title ? '<p class="teacher-info__title">' + esc(t.title) + '</p>' : '') +
      (t.bio ? '<p class="teacher-info__bio">' + esc(t.bio).replace(/&lt;strong&gt;(.*?)&lt;\/strong&gt;/g, '<strong>$1</strong>') + '</p>' : '') +
      '</div>';
  }

  /* ---------- 服務 ---------- */
  function renderService(s) {
    return '<div class="service fade-in visible">' +
  ...
  function renderWork(w) {
    ...
    if (!img) return '';
    return '<div class="gallery__item fade-in visible">' +
      '<picture><img src="' + esc(img) + '" alt="' + esc(w.caption || '陶茶雅舍手作陶藝作品') + '" loading="lazy" width="800" height="600" decoding="async">' +
      '</picture></div>';
  }
  ...
  function renderMedia(m) {
    ...
    return '<article class="media-card fade-in visible">' +
      '<div class="media-card__type">' + icon + ' ' + esc(m.type) + '</div>' +
      '<div class="media-card__body">' +
      '<h3 class="media-card__title">' + esc(m.title) + '</h3>' +
      (m.description ? '<p class="media-card__desc">' + esc(m.description) + '</p>' : '') +
      (m.link ? '<a href="' + esc(m.link) + '" target="_blank" rel="noopener" class="media-card__link">' + linkText + '</a>' : '') +
      '</div></article>';
  }
  ...
  function renderContact(s) {
    ...
    if (contact && (s.address || s.phone || s.facebook || s.hours)) {
      contact.innerHTML =
        ...
        '<p class="contact__value">' + esc(s.address || '').replace('\n', '<br>') + '</p></div></div>' +
        '<div class="contact__item"><div class="contact__icon">📞</div><div>' +
        '<p class="contact__label">PHONE</p>' +
        '<p class="contact__value"><a href="' + esc(s.phone_href || 'tel:' + s.phone) + '">' + esc(s.phone) + '</a></p></div></div>' +
        '<div class="contact__item"><div class="contact__icon">👍</div><div>' +
        '<p class="contact__label">FACEBOOK</p>' +
        '<p class="contact__value"><a href="' + esc(s.facebook) + '" target="_blank" rel="noopener">陶茶雅舍 粉絲專頁｜FB 訊息聯繫</a></p></div></div>' +
        '<div class="contact__item"><div class="contact__icon">🕐</div><div>' +
        '<p class="contact__label">OPEN HOURS</p>' +
        '<p class="contact__value">' + esc(s.hours).replace('\n', '<br>') + '</p></div></div>';
    }

    /* 地圖 */
    if (s.map_query) {
      var iframe = document.querySelector('.contact__map iframe');
      if (iframe) iframe.src = 'https://www.google.com/maps?q=' + encodeURIComponent(s.map_query) + '&output=embed';
    }
  ...
  function renderCoursePage(course) {
    ...
    if (hero) {
      ...
      if (h1) {
        var subtitleHtml = course.subtitle
          ? '<br><span style="font-size:0.7em;color:var(--color-accent);">（' + esc(course.subtitle) + ' ✦）</span>'
          : '';
    ...
    if (meta) {
      ...
      if (course.time) html += '<dt>🕐 時間</dt><dd>' + esc(course.time).replace('\n', '<br>') + '</dd>';
      if (course.capacity) html += '<dt>👥 名額</dt><dd><strong style="color:var(--color-accent);">' + esc(course.capacity) + '</strong>　小班精緻教學</dd>';
      if (course.fee) html += '<dt>💰 費用</dt><dd>' + esc(course.fee) + '</dd>';
      if (course.bonus) html += '<dt>🎁 加碼</dt><dd>' + esc(course.bonus) + '</dd>';
      if (course.location) html += '<dt>📍 地點</dt><dd>' + esc(course.location).replace('\n', '<br>') + '</dd>';
      if (course.signup_url) html += '<dt>📝 報名</dt><dd><a href="' + esc(course.signup_url) + '" target="_blank" rel="noopener" style="color:var(--color-primary);">點此填寫報名表 →</a></dd>';
      meta.innerHTML = html;
    }

    /* 報名按鈕 */
    if (course.signup_url) {
      document.querySelectorAll('.signup-card a.btn--primary, .section--dark a.btn--primary, .sticky-cta').forEach(function (a) {
    ...
    if (course.date) {
      ...
      if (cta && course.fee) {
        cta.innerHTML = '立即報名 ' + esc(course.title) + ' <span>' + String(course.fee).replace(' / 人', '') + '</span> →';
      }
    }
  }
    ...
    if (data.exhibitions && data.exhibitions.length) {
      ...
      if (timeline) {
        var items = data.exhibitions.map(function (e, i) {
          ...
          return '<div class="timeline__item' + extra + '"><span class="timeline__year">' + esc(e.year) + '</span>' + esc(e.title) + '</div>';
        });
